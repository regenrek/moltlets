...
let
  cfg = config.services.openclawCattle;
  ...
  clawPkg = openclawPkgs.openclaw-gateway;
in
{
  options.services.openclawCattle = {
    enable = lib.mkEnableOption "OpenClaw cattle (ephemeral single-task runner)";
