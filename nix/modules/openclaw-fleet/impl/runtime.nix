    ...
    in
      {
        ...
        value = {
          ...
          environment =
            if envDupes != []
            then throw "services.openclawFleet.botProfiles.${b}.env has keys that conflict with secret env vars: ${lib.concatStringsSep "," envDupes}"
            else {
              OPENCLAW_NIX_MODE = "1";
        ...
        in
          map (id:
            let
              ...
              targetPath = toString (spec.targetPath or "");
              _ = validateTargetPath {
                context = "services.openclawFleet.botProfiles.${b}.secretFiles.${id}.";
  ...
  secretFileSecrets =
    let
      go = acc: entry:
        ...
        in
          if prev != null
          then builtins.throw "secretFiles: duplicate secretName ${key} (must be unique across services.openclawFleet.secretFiles and botProfiles.<bot>.secretFiles)"
