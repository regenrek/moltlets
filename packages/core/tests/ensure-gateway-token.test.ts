...
describe("ensure-gateway-token script", () => {
  it("writes token file atomically in destination dir", () => {
    ...
    const outDir = fs.mkdtempSync(path.join(os.tmpdir(), "clawdlets-gateway-token-"));
    ...
    execFileSync("bash", [scriptPath], {
      env: {
        ...process.env,
        CLAWDLETS_GATEWAY_ENV_FILE: outEnvFile,
        CLAWDLETS_BOT_USER: user,
        CLAWDLETS_BOT_GROUP: group,
      },
      stdio: "pipe",
    });
    ...
    const leftover = fs.readdirSync(outDir).filter((name) => name.startsWith(".clawdlets-gateway-token."));
