  ...
  const addChannelToken = (payload: { channel: string; envVar: string; path: string; value: unknown }) => {
    ...
    if (envVar && envVar !== payload.envVar) {
      warnings.push({
        ...
        suggestion: `Use \${${payload.envVar}} for ${payload.channel} and map it in fleet.secretEnv or hosts.<host>.gateways.${gatewayId}.profile.secretEnv.`,
      });
    }
    if (!envVar) {
      warnings.push({
        ...
        suggestion: `Replace with \${${payload.envVar}} and map it in fleet.secretEnv or hosts.<host>.gateways.${gatewayId}.profile.secretEnv.`,
      });
    }
  ...
  const addHookToken = (payload: { envVar: string; path: string; value: unknown; label: string }) => {
    ...
    if (envVar && envVar !== payload.envVar) {
      warnings.push({
        ...
        path: payload.path,
        message: `Unexpected env ref at ${payload.path}: ${trimmed}`,
        suggestion: `Use \${${payload.envVar}} for ${payload.label} and map it in fleet.secretEnv or hosts.<host>.gateways.${gatewayId}.profile.secretEnv.`,
      });
    }
    if (!envVar) {
      warnings.push({
  ...
  const addSkillApiKey = (payload: { skill: string; path: string; value: unknown }) => {
    ...
    if (envVar && envVar !== expectedEnvVar) {
      warnings.push({
        ...
        path: payload.path,
        message: `Unexpected env ref at ${payload.path}: ${trimmed}`,
        suggestion: `Use \${${expectedEnvVar}} and map it in fleet.secretEnv or hosts.<host>.gateways.${gatewayId}.profile.secretEnv.`,
      });
    }
    if (!envVar) {
      warnings.push({
        ...
        suggestion: `Replace with \${${expectedEnvVar}} and map it in fleet.secretEnv or hosts.<host>.gateways.${gatewayId}.profile.secretEnv.`,
      });
    }
