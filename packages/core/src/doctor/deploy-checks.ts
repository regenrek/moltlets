  ...
  if (clawletsHostCfg) {
    ...
    if (secretsPlan && secretsPlan.missingSecretConfig.length > 0) {
      for (const m of secretsPlan.missingSecretConfig.slice(0, 10)) {
        const detail =
          m.kind === "envVar"
            ? `missing mapping gateway=${m.gateway} envVar=${m.envVar} (set fleet.secretEnv.${m.envVar} or fleet.gateways.${m.gateway}.profile.secretEnv.${m.envVar})`
    ...
    if (gatewaysForSecrets.length > 0) {
      ...
      push({ status: "warn", label: "required secrets", detail: "(fleet gateways list missing; cannot validate per-gateway secrets)" });
    }
