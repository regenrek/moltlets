...
describe("config set", () => {
  ...
  it("show prints JSON", async () => {
    const configObj = createDefaultClawletsConfig({ host: "alpha", gateways: ["maren"] });
  ...
  it("set fails on invalid JSON", async () => {
    ...
    const { config } = await import("../src/commands/config");
    await expect(
      config.subCommands.set.run({ args: { path: "hosts.alpha.gatewaysOrder", "value-json": "nope" } } as any),
    ).rejects.toThrow(
  ...
  it("set rejects missing value flags", async () => {
    ...
    const { config } = await import("../src/commands/config");
    await expect(config.subCommands.set.run({ args: { path: "hosts.alpha.gatewaysOrder" } } as any)).rejects.toThrow(
      /set requires/i,
    );
  });
