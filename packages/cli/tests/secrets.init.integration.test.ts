...
const buildPlan = (overrides: Record<string, unknown>) => {
  ...
  const optional =
    ...
    secretNamesAll
      ...
      .map((name) => ({ name, kind: "env", scope: "gateway", source: "custom", optional: true }));
  return {
    gateways: [],
...
describe("secrets init", () => {
  ...
  it("fails when hosts.<host>.gatewaysOrder is empty", async () => {
    const repoRoot = fs.mkdtempSync(path.join(tmpdir(), "clawlets-secrets-"));
    const layout = getRepoLayout(repoRoot);
    const config = makeConfig({
      hostName: "alpha",
      hostOverrides: { ...baseHost, tailnet: { mode: "none" }, gatewaysOrder: [], gateways: {} },
    });
    const hostCfg = config.hosts.alpha;
    loadHostContextMock.mockReturnValue({ layout, config, hostName: "alpha", hostCfg });
    const { secretsInit } = await import("../src/commands/secrets/init.js");
    await expect(secretsInit.run({ args: { host: "alpha" } } as any)).rejects.toThrow(/gatewaysOrder is empty/i);
  });
  ...
  it("fails when discord secret config is missing", async () => {
    ...
    buildFleetSecretsPlanMock.mockReturnValue(buildPlan({
      ...
      secretNamesAll: [],
      secretNamesRequired: [],
      missingSecretConfig: [{ kind: "envVar", gateway: "maren", envVar: "DISCORD_BOT_TOKEN", sources: ["custom"], paths: [] }],
    }));
    const { secretsInit } = await import("../src/commands/secrets/init.js");
    ...
    buildFleetSecretsPlanMock
      .mockReturnValueOnce(
        buildPlan({
          ...
          secretNamesAll: [],
          secretNamesRequired: [],
          missingSecretConfig: [{ kind: "envVar", gateway: "maren", envVar: "DISCORD_BOT_TOKEN", sources: ["channel"], paths: [] }],
        }),
      )
