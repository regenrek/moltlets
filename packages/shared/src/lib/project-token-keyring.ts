...
function normalizeLabel(rawLabel: string, value: string): string {
  ...
  const head = trimmedValue.slice(0, 6);
  const tail = trimmedValue.slice(-4);
...
export function resolveActiveProjectTokenEntry(params: {
  ...
  const byId = requested ? params.keyring.items.find((row) => row.id === requested) : undefined;
...
export function maskProjectTokenKeyringRaw(raw: unknown): string {
  const parsed = parseProjectTokenKeyring(raw);
  const masked = {
    items: parsed.items.map((entry) => ({
      ...entry,
      value: maskProjectToken(entry.value),
    })),
  } satisfies ProjectTokenKeyring;
  return serializeProjectTokenKeyring(masked);
}
