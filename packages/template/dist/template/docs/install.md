...
- `secrets/.sops.yaml`
  - creation rules + recipients for `secrets/hosts/clawdlets-fleet-mj-2/*.yaml` (generated by `clawdlets secrets init`)
- `secrets/hosts/clawdlets-fleet-mj-2/`
...
- `.clawdlets/extra-files/clawdlets-fleet-mj-2/var/lib/sops-nix/key.txt`
...
```nix
({ ... }: {
  clawdlets.hostName = "clawdlets-fleet-mj-2";
})
```
...
```bash
SOPS_AGE_KEY_FILE=.clawdlets/keys/operators/<you>.agekey EDITOR=vim sops edit secrets/hosts/clawdlets-fleet-mj-2/admin_password_hash.yaml
```
...
```bash
ssh admin@<tailscale-ip-of-clawdlets-fleet-mj-2>
```
