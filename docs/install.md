...
If you’re using clawlets as intended (CLI-first; no need to clone this repo):
...
These paths live in the project repo created by `clawlets project init`.

- `fleet/clawlets.json` (canonical config: hosts + fleet/bots + optional clawdbot config passthrough)
- `flake.nix` / `flake.lock` (build graph; imports clawlets framework; usually no edits)
- `secrets/` (committed; sops-encrypted secrets + sops rules)
- `.clawlets/` (gitignored runtime: operator keys + nixos-anywhere extra-files + image outputs)
- `fleet/workspaces/` (AGENTS/SOUL/TOOLS/IDENTITY/USER/HEARTBEAT seeded into workspaces)
- `.clawlets/infra/opentofu/<host>/terraform.tfstate` (gitignored; keep single-operator policy)
...
- `secrets/.sops.yaml`
  - creation rules + recipients for `secrets/hosts/clawdbot-fleet-host/*.yaml` (generated by `clawlets secrets init`)
...
- `.clawlets/extra-files/clawdbot-fleet-host/var/lib/sops-nix/key.txt`
  - host age key installed to `/var/lib/sops-nix/key.txt` during nixos-anywhere

## Disk device (required)

Disk device is read from `hosts.<host>.diskDevice` in `fleet/clawlets.json` (or via `clawlets host set --disk-device ...`).
...
```bash
SOPS_AGE_KEY_FILE=.clawlets/keys/operators/<you>.agekey EDITOR=vim sops edit secrets/hosts/clawdbot-fleet-host/admin_password_hash.yaml
```

Note: run `clawlets secrets init` first (it generates `.clawlets/keys/operators/<you>.agekey` and `secrets/.sops.yaml`). If sops fails with “no identity matched any of the recipients”, re-run `clawlets secrets init --yes`.
...
```bash
just clawlets-dev-install
clawlets --help
```

Note: provisioning is handled by the CLI; state is stored under `.clawlets/infra/opentofu/<host>` (gitignored).
...
```bash
clawlets bootstrap --flake github:<owner>/<repo>
```
...
Add a second host by adding another host entry in `fleet/clawlets.json`, then run `clawlets infra apply --host <host>`.
...
Note: `clawlets bootstrap` clears these entries automatically after install.
