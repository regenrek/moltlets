...
Clawlets does not invent a second routing/channels schema. Use clawdbot’s config schema directly.

## Where to put config

- `fleet/clawlets.json` → `fleet.bots.<bot>.clawdbot` (raw clawdbot config object)

Clawlets invariants always win:
...
Clawlets treats Clawdbot security as deploy-critical by default: insecure “open” policies should not ship accidentally.

- `clawlets doctor` runs a static Clawdbot security lint per bot (config-only; no gateway needed).
- `clawlets server audit` runs `clawdbot security audit --json` per bot over SSH (fails on critical findings).
- `clawlets clawdbot harden` applies safe defaults to `fleet/clawlets.json` (dry-run unless `--write`).
...
- pinned schema is bundled with clawlets and used by CLI validation + default UI editor
- live schema can be fetched from a running gateway in the web UI ("Use live schema")
  ...
  - CLI: `clawlets clawdbot schema fetch --host <host> --bot <bot>`
...
Use env var references in clawdbot config and wire them to sops secret names in `fleet/clawlets.json`:
