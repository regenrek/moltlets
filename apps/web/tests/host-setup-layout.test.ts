...
describe("host setup layout", () => {
  it("flattens step wrappers and keeps section anchors", () => {
    ...
    expect(source).toContain("<StepperContent key={step.id} value={step.id} forceMount>")
    ...
    expect(source).not.toContain("Route.useSearch()")
    expect(source).not.toContain("setup.setStep(")
  })
