...
function OpenSshExposureBanner() {
  ...
  const openHosts = React.useMemo<OpenHost[]>(() => {
    ...
    return hosts.flatMap((host) => {
      ...
      if (
        host.sshExposureMode === "bootstrap" ||
        host.sshExposureMode === "public"
      ) {
  ...
  const targetHost =
    openHosts.find((host) => host.name === activeHost)?.name ??
    openHosts[0]?.name;
  ...
  const settingsPath = targetHost
    ? `${buildHostPath(projectSlug, targetHost)}/settings`
    : "";
    ...
    <div className="border-b border-destructive/40 bg-destructive/10 text-destructive">
      <div className="mx-auto flex max-w-screen-2xl flex-wrap items-center gap-2 px-4 py-2 text-sm sm:px-6">
        ...
        <span className="text-destructive/80">
          Complete the post-bootstrap checklist to switch SSH to tailnet and
          ...
          <Button
            ...
            render={
              <Link
                ...
                params={{ projectSlug, host: setupLinkHost }}
              />
            }
          >
            Open checklist
