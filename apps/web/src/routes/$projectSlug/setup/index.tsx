...
import { slugifyProjectName, buildHostPath } from "~/lib/project-routing"
import { useSetupModel } from "~/lib/setup/use-setup-model"
import {
  clawletsConfigQueryOptions,
  deployCredsQueryOptions,
...
export const Route = createFileRoute("/$projectSlug/setup/")({
  ...
  component: SetupIndexPage,
})

function SetupIndexPage() {
  const { projectSlug } = Route.useParams()
  const search = Route.useSearch()
  const setup = useSetupModel({ projectSlug, search })
  ...
  const hostCfg = selectedHost ? (setup.config?.hosts as any)?.[selectedHost] : null
  ...
  const deployHref = selectedHost ? `${buildHostPath(projectSlug, selectedHost)}/deploy` : null
            ...
            return (
              <SetupSection key={step.id} value={step.id} index={index} title={step.title} status={step.status}>
                <SetupStepHost
                  ...
                  config={setup.config}
                  selectedHost={selectedHost}
                  onSelectHost={setup.setSelectedHost}
