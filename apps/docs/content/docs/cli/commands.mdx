...
Provision Hetzner + install NixOS.
...
- `status` — systemd status for Clawdbot services
- `logs` — journalctl logs for Clawdbot services
- `restart` — restart one or more Clawdbot units
- `audit` — tailscale + service checks + security audit
- `update` — updater apply/status/logs
- `channels` — run `clawdbot channels` over SSH
...
- `show` — print config
- `validate` — schema validation + rendered Clawdbot config
...
Infrastructure commands wrap OpenTofu and image workflows for Hetzner.

### `clawlets infra`

Hetzner OpenTofu operations.
...
Helpers around OpenClaw/Clawdbot validation and policy.

### `clawlets clawdbot`

OpenClaw/Clawdbot helpers.

Subcommands:

- `schema fetch` — fetch live config schema from gateway
- `harden` — apply default hardening to bot config
