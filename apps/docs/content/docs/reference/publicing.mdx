---
title: Going public
description: OSS-safe publishing checklist.
---

This checklist is for publishing a project repo without leaking secrets or host
details. It is intentionally conservative and assumes the repo will be public.
...
The most reliable path is a clean export so you do not accidentally publish commit history:
...
Users can run `CLAWLETS_INTERACTIVE=1 clawlets secrets init` to generate local operator
keys, create `secrets/` (encrypted), and generate `.clawlets/extra-files/<host>/...`.
