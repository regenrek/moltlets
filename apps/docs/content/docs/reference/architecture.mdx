...
- **Nix modules**: `clawletsProjectHost`, `clawdbotFleet`, `clawletsSelfUpdate`, `clfOrchestrator`.
- **Per-bot gateway**: one Clawdbot gateway per bot (user `bot-<id>`, state `/srv/clawdbot/<id>`).
...
renders bot config into `/run/secrets`, and sops-nix materializes secrets at runtime.

- **Config**: `fleet/clawlets.json` → Nix modules → rendered bot config in `/run/secrets/rendered/clawdbot-<bot>.json`.
...
- **Gateway auth**: per-bot token stored in `/srv/clawdbot/<bot>/credentials/gateway.env`.
