...
- **Per-bot gateway**: one OpenClaw gateway per bot (user `bot-<id>`, state `/srv/openclaw/<id>`).
...
- **Config**: `fleet/clawlets.json` → Nix modules → rendered bot config in `/run/secrets/rendered/openclaw-<bot>.json`.
...
- **Gateway auth**: per-bot token stored in `/srv/openclaw/<bot>/credentials/gateway.env`.
