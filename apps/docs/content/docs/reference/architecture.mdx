...
- **clf-cattle (plugin repo)**: `@clawlets/plugin-cattle` + `clf` queue/orchestrator stack (opt-in).
...
- **Orchestrator (`clf-orchestrator`, plugin)**: job queue + cattle worker, socket `/run/clf/orchestrator.sock`.
...
- Setup details: [CLF Orchestrator](/plugins/clf-orchestrator)
