...
If gateways cannot connect to the control plane, check the Unix socket permissions:

- socket path: `/run/clf/orchestrator.sock`
- expected group: `clf-gateways`

Confirm the gateway service user is in the right group and that the socket is `0660`.
