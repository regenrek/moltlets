...
- gateway scope: `fleet.gateways.<id>.profile.secretEnv` and `fleet.gateways.<id>.profile.secretFiles`

Secret file targets:

- `fleet.secretFiles.*.targetPath` must be under `/var/lib/clawlets/`
- `fleet.gateways.<id>.profile.secretFiles.*.targetPath` must be under `/var/lib/clawlets/secrets/gateways/<id>/`

## `${ENV}` wiring + autowire

Clawlets detects `${ENV_VAR}` refs inside `fleet.gateways.<id>.openclaw`, plus known channel tokens,
...
- `discord_token_<id>` (default mapping via `fleet.gateways.<id>.profile.secretEnv.DISCORD_BOT_TOKEN`)
