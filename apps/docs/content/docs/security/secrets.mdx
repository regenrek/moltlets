...
- `.clawlets/keys/operators/<operator>.agekey` (operator private key; local only)
...
- bot scope: `hosts.<host>.bots.<botId>.profile.secretEnv` and `hosts.<host>.bots.<botId>.profile.secretFiles`

Secret file targets:

- `fleet.secretFiles.*.targetPath` must be under `/var/lib/clawlets/`
- `hosts.<host>.bots.<botId>.profile.secretFiles.*.targetPath` must be under `/var/lib/clawlets/secrets/gateways/<botId>/`

## `${ENV}` wiring + autowire

Clawlets detects `${ENV_VAR}` refs inside `hosts.<host>.bots.<botId>.openclaw`, plus known channel tokens,
...
- `.clawlets/extra-files/<host>/...` (host key + encrypted host secrets) for first install
...
- `discord_token_<botId>` (default mapping via `hosts.<host>.bots.<botId>.profile.secretEnv.DISCORD_BOT_TOKEN`)
