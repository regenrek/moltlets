...
Clawlets detects `${ENV_VAR}` refs inside `fleet.bots.<bot>.openclaw`, plus known channel tokens,
...
- LLM API keys wired via env vars referenced in OpenClaw config, then mapped in `fleet.secretEnv` or per‑bot overrides
