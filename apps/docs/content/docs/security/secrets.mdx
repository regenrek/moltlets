...
Clawlets detects `${ENV_VAR}` refs inside `fleet.bots.<bot>.clawdbot`, plus known channel tokens,
...
- LLM API keys wired via env vars referenced in Clawdbot config, then mapped in `fleet.secretEnv` or per‑bot overrides
- hook tokens mapped to `CLAWDBOT_HOOKS_TOKEN` and `CLAWDBOT_HOOKS_GMAIL_PUSH_TOKEN`
- skill keys mapped to `CLAWDBOT_SKILL_<SKILL>_API_KEY`
