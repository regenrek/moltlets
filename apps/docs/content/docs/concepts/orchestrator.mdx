...
without ever holding Hetzner credentials or long-lived provider keys. The separation
between `clawlets` (operator/admin CLI) and `clf` (bot-facing CLI) keeps privileges
clear and makes it easier to audit who can do what.
...
The Clawlets framework ships a NixOS module for the orchestrator. Enable it in your
host config and wire the required secrets.
