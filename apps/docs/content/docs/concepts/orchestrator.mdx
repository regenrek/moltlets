...
The orchestrator is the gateway-facing control plane. Gateways request work through `clf jobs`
...
- `clf`: gateway-facing control plane (jobs queue)

## Components

The orchestrator is composed of a queue, a daemon, and a gateway-facing CLI:

- `packages/clf/queue`: SQLite queue + schemas (idempotency, leases, retry).
- `packages/clf/orchestrator`: daemon + worker loop + handlers.
- `packages/clf/cli`: `clf` CLI (gateways call this).

## Transport + auth (default)

Gateways and cattle talk to the orchestrator over different transports with different
constraints. Gateways use a Unix socket protected by filesystem permissions, and cattle
fetch their runtime environment over tailnet-only HTTP.

- Gateway → orchestrator: Unix socket HTTP
...
- module: `nix/openclaw/modules/clf-orchestrator.nix`
...
Gateways enqueue jobs via the `clf` CLI. The queue enforces idempotency so a single
...
Notes
- Gateways use `clf`. Do not grant gateways access to the `clawlets` admin CLI.
