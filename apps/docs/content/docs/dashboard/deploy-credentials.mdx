...
- `TAILSCALE_AUTH_KEY` (Tailscale, required for tailnet activation + lockdown)
...
<Steps>
  ...
  <Step>
    ...
    If you use Tailscale lockdown, set an active `TAILSCALE_AUTH_KEY`.
  </Step>
  <Step>
...
[Tailscale auth key setup](/dashboard/tailscale-auth-key)
...
In host Setup, GitHub token is required from deploy credentials.
If you enable Tailscale lockdown (or set `hosts.<host>.tailnet.mode=tailscale`), a Tailscale auth key is also required.
