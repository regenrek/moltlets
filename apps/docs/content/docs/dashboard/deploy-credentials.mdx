...
- `GITHUB_TOKEN` (required for Setup bootstrap/apply when evaluation repo is private)
- `SOPS_AGE_KEY_FILE`
...
- Confirm required key is set: `GITHUB_TOKEN` (required when the evaluation repo is private)
