...
that Nix uses as the evaluation source, and to fetch GitHub-hosted flake inputs during
...
- **Evaluation repo**: the repo Nix reads from. Usually this is the project repo’s
  <code>origin</code> remote, unless you set <code>baseFlake</code> to a different GitHub repo.

Dashboard project creation only sets up the local project folder and git repo; it does not
create a remote GitHub repo for you. If you need remote/CI flow, create a private GitHub repo,
add it as <code>origin</code>, and push manually.

If the evaluation repo is private, the Setup <code>Provider Tokens</code> step stays blocked
until the token can authenticate.

That token lets the runner:

- verify GitHub API visibility for your evaluation repo, and
- pass GitHub credentials to Nix when it evaluates or pulls <code>github:</code> flake inputs.
...
- the current repo’s <code>origin</code> remote when <code>baseFlake</code> is empty
Does Clawlets ever change it for you?

No. The code only reads `baseFlake` to resolve where to evaluate the flake; it does not
rewrite it during bootstrap or setup. You change it only when you edit `fleet/clawlets.json`
yourself (or via `clawlets config set baseFlake ...`).

Why is this option there?

It is an explicit escape hatch when the project repo is not where Nix evaluation should
pull inputs from (for example when sharing a template repo, pinning a different remote repo,
or temporarily overriding for one-off bootstrap flows with `--flake`).
...
that contains <code>fleet/clawlets.json</code> and <code>secrets/</code>.
...
- Evaluation source is in <code>fleet/clawlets.json</code> field <code>baseFlake</code>; fallback is
  project repo <code>origin</code>.
