...
If you need to bypass auth locally while wiring the UI, you can disable it for the
dev deployment:
...
Then also set `VITE_CLAWLETS_AUTH_DISABLED=true` in `.env.local` so the web app matches
the Convex setting.
