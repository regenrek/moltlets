...
3. Runner creates/loads the project repo in that local path. The dashboard does not create
   any extra bootstrap repo for you.
4. Start runner with the token/command shown in the dialog.
5. Wait for project init/import run to finish.
6. For remote/CI workflows, create a private GitHub repo, add it as <code>origin</code>, and
   push the local project repo manually.
7. Open host setup at `/<projectSlug>/hosts/<host>/setup`.
...
<Callout type="warning" title="Connect runner first">
  Stage 2 starts with a required **Connect Runner** step. A runner heartbeat is treated as
  fresh for about 30 seconds. Until the runner is online, all later steps stay locked.
  See [Runner Agent](/operations/runner-agent).
</Callout>

Step order and gating:

1. `Connect Runner` (`runner`): requires a fresh online runner heartbeat.
2. `Add First Host` (`host`): choose an existing host or add one.
3. `Server Access` (`connection`): requires `provisioning.adminCidr` and at least one `fleet.sshAuthorizedKeys` entry.
4. `Provider Tokens` (`creds`): requires `HCLOUD_TOKEN`, `GITHUB_TOKEN`, and `SOPS_AGE_KEY_FILE`.
5. `Server Passwords` (`secrets`): requires successful run `secrets_verify_bootstrap`.
6. `Install Server` (`deploy`): requires successful bootstrap run (`bootstrap`).
7. `Secure and Verify` (`verify`): optional post-bootstrap hardening checklist.

Project security surfaces used during this stage:

- [SSH keys](/dashboard/ssh-keys) (`/$projectSlug/security/ssh-keys`)
- [Deploy credentials](/dashboard/deploy-credentials) (`/$projectSlug/security/api-keys`)
