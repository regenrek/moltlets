...
5. Open host setup at `/<projectSlug>/hosts/<host>/setup`.
   Created projects are initialized locally on the runner; no dedicated bootstrap repository is
   created by Clawlets. For remote/CI usage, create a private GitHub repository yourself,
   set it as `origin`, and push the local project repo manually.
