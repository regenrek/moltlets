...
- **Operators / end users (recommended):** start with [Quickstart](/quickstart)
- **CLI users:** install CLI here, then use [CLI overview](/cli) and [Command reference](/cli/commands)
- **Contributors:** use [Contributing](/contribution) for local dev setup
...
For fresh workstations, prefer the canonical quickstart command:

```bash
clawlets quickstart
```

Quickstart handles Nix detection/install, Convex bootstrap, and local UI startup.
See [Quickstart](/quickstart) for onboarding and [Operations quickstart](/operations/quickstart) for detailed flags.

## Install Nix (required)

Clawlets shells out to `nix` for tooling (`age-keygen`, `sops`, `mkpasswd`, `nixos-anywhere`, builds/evals).
Install Nix on the workstation that runs the dashboard/runner.
...
Dedicated install guide: [Install Nix](/operations/install-nix)

If the dashboard/runner cannot find `nix`, set `NIX_BIN` in `<runtimeDir>/env` to the full path (common on macOS: `/nix/var/nix/profiles/default/bin/nix`).
...
- `<runtimeDir>/` (gitignored runtime: operator keys + extra‑files + infra state + images)
...
## Deterministic dev env (contributors)
...
This is optional for operators using quickstart. It is mainly for contributor/dev workflows.
...
- `secrets/`
  ...
  - `secrets/hosts/<host>/tailscale_auth_key.yaml` (required when tailnet.mode=tailscale; see [Tailscale auth key setup](/dashboard/tailscale-auth-key))
  ...
  - `keys/operators/hosts/<host>/<you>.agekey` (host-scoped operator key; created by `clawlets secrets init`)
...
## Deploy credentials (`<runtimeDir>/env`)
...
- `SOPS_AGE_KEY_FILE` (optional override; use host-scoped path)
...
<Callout type="warning" title="AWS is experimental">
  ...
  <code>&lt;runtimeDir&gt;/env</code> and follow [AWS provider notes](/providers/aws).
</Callout>
