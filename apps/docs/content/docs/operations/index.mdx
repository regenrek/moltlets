...
- edit `fleet/clawlets.json` → `fleet.gateways.<bot>.openclaw` (routing/tools/etc)
- edit `fleet/clawlets.json` → `fleet.gateways.<bot>.channels` (typed channels)
...
```bash
clawlets gateway add --gateway <id>
```

2) Add secret + config:

- `fleet.gateways.<id>.profile.secretEnv.DISCORD_BOT_TOKEN = "discord_token_<id>"`
- channel config: `fleet.gateways.<id>.channels.discord = { enabled: true, token: "${DISCORD_BOT_TOKEN}" }`
...
```bash
clawlets config set --path fleet.gateways.maren.skills.allowBundled --value-json '["github","brave-search"]'
```
3) If it needs secrets: add `secrets/hosts/<host>/<secret>.yaml` and reference in `fleet.gateways.<bot>.skills.entries."<skill>".apiKeySecret`
4) Sync + deploy:
