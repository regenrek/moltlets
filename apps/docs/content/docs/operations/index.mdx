...
- edit `fleet/clawlets.json` → `hosts.<host>.bots.<botId>.openclaw` (routing/tools/etc)
- edit `fleet/clawlets.json` → `hosts.<host>.bots.<botId>.channels` (typed channels)
...
```bash
clawlets gateway add --host <host> --gateway <botId>
```

2) Add secret + config:

- `hosts.<host>.bots.<botId>.profile.secretEnv.DISCORD_BOT_TOKEN = "discord_token_<botId>"`
- channel config: `hosts.<host>.bots.<botId>.channels.discord = { enabled: true, token: "${DISCORD_BOT_TOKEN}" }`
...
```bash
clawlets config set --path hosts.<host>.bots.maren.skills.allowBundled --value-json '["github","brave-search"]'
```
3) If it needs secrets: add `secrets/hosts/<host>/<secret>.yaml` and reference in `hosts.<host>.bots.<botId>.skills.entries."<skill>".apiKeySecret`
4) Sync + deploy:
...
Gateway/job inspection:

```bash
sudo -u gateway-maren clf jobs list --json
sudo -u gateway-maren clf jobs show --job-id <jobId> --json
```
...
If enabled (`services.openclawFleet.githubSync.enable = true`), each bot writes:

- `/srv/openclaw/<bot>/workspace/memory/github/prs.md`
- `/srv/openclaw/<bot>/workspace/memory/github/issues.md`
