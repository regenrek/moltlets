...
- edit `fleet/clawlets.json` → `fleet.bots.<bot>.openclaw` (routing/tools/etc)
- edit `fleet/clawlets.json` → `fleet.bots.<bot>.channels` (typed channels)
...
- channel config: `fleet.bots.<id>.channels.discord = { enabled: true, token: "${DISCORD_BOT_TOKEN}" }`
