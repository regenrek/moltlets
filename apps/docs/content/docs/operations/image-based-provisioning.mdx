...
Image‑based provisioning lets you boot hosts from a prebuilt NixOS image and skip
`nixos-anywhere` during bootstrap. This is useful when you want faster host creation
or a repeatable image pipeline.

## Reality check (providers)
...
AWS supports a direct import path from S3 (`ec2 import-image`) which fits the Phase 2
image/AMI workflow.
...
## AWS path (Phase 2-first)

For AWS, the canonical path is S3 upload + AMI import:
...
clawlets bootstrap --host <host> --mode image
```

Use `clawlets image aws upload` and `clawlets image aws import` separately if you need
manual control over the S3 key or import timing.
