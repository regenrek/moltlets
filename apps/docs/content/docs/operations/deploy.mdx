...
<Callout type="info" title="Execution mode matters">
  ...
  The current Create Project dialog scaffolds <code>executionMode=local</code> projects, which require the dashboard server to have repo access for some operations (scaffolding and workspace docs).
</Callout>
