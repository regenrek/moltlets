...
## `spawn nix ENOENT` / `nix not found (install Nix first)`

The runner host is missing `nix` (or cannot resolve it in PATH).

Install Nix on that machine with the checksum-verified flow in
[Install Nix](/operations/install-nix).

Then restart runner and retry. If shell PATH differs from service PATH, set `NIX_BIN`
explicitly in `<runtimeDir>/env`.
...
```bash
nix run --impure nixpkgs#opentofu -- -chdir=<runtimeDir>/infra/opentofu/<host> plan
```
...
```bash
ssh -t <host> "sudo systemctl status openclaw-melinda --no-pager"
```
...
```bash
clawlets server restart --target-host <host> --unit openclaw-melinda.service
```
...
See: [Tailscale auth key setup](/dashboard/tailscale-auth-key)
...
## Experimental plugin issues

CLF/cattle troubleshooting moved to the external experimental plugin docs.
See [Plugins overview](/plugins) and the linked `clf-cattle` repository.
