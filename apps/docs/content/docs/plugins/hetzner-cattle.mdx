...
```json
{
  ...
  "hosts": { "openclaw-fleet-host": { "enable": true } }
}
```
...
- Socket access:
  - operator user in group `clf-gateways` (or run as root).
...
- Gateways should use `clf jobs ...` directly; see [Orchestrator](/concepts/orchestrator).
...
- `missing secretEnv mapping for <ENV_VAR>`: set `fleet.secretEnv.<ENV_VAR>` or per-gateway override.
