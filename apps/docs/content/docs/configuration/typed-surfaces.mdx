...
flowchart LR
  ...
  S --> C["nix eval: nix/infra/lib/fleet-config.nix"]
...
- `nix/infra/lib/fleet-config.nix` (maps JSON config → Nix module inputs)
- `nix/openclaw/modules/openclaw-fleet/impl/gateway-config.nix` (builds final OpenClaw config per gateway)
