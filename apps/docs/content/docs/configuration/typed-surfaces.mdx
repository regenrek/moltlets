...
Clawlets validates the **effective** OpenClaw config for each bot against a **pinned
OpenClaw schema snapshot**.

This is the guardrail that keeps typed surfaces and passthrough aligned with upstream:

- if a key is unknown (or the type is wrong), validation fails fast
- validation is offline (no live gateway required)

Schema snapshot (for contributors):

- `packages/core/src/assets/clawdbot-config.schema.json` (generated; name kept for compatibility)
- regen: `pnpm clawlets:openclaw-assets -- --src /path/to/openclaw`
