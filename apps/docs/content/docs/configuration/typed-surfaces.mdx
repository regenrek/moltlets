...
- **OpenClaw passthrough** (`fleet.bots.<bot>.openclaw`) for advanced/edge cases

In addition, each bot has a **profile** (`fleet.bots.<bot>.profile`) that controls the
...
- `fleet.bots.<bot>.openclaw` (raw JSON object merged into the OpenClaw root config)

<Callout type="warning" title="Do not nest typed surfaces under openclaw.*">
  Do not set <code>fleet.bots.&lt;bot&gt;.openclaw.channels</code> (or agents/hooks/skills/plugins).
...
`fleet.bots.<bot>.profile` configures runtime behavior that is enforced by the Nix layer:
