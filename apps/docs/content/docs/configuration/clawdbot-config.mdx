...
description: Use Clawdbot's schema directly.
---

Clawlets does not invent a second routing or channels schema. You use Clawdbot’s
configuration directly, and Clawlets applies a small set of invariants.

## Where to put config

Clawdbot config lives inside the fleet file, and it is stored per bot:
...
any conflicting values in the raw Clawdbot config:
...
Clawlets includes Clawdbot security lint and treats open policies as deploy‑critical
findings.
...
defaults with `clawlets clawdbot harden` (dry‑run unless `--write`). The web editor
...
- `clawlets clawdbot schema fetch --host <host> --bot <bot>`

The UI also shows schema drift between the pinned schema, the project’s pinned
`nix-clawdbot` rev, and upstream `nix-clawdbot` main.
...
Use env var references in Clawdbot config and wire them to sops secret names in
