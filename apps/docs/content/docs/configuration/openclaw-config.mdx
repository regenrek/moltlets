...
- `fleet.bots.<bot>.openclaw` (raw OpenClaw config object)
...
- Discord: set `fleet.bots.<bot>.channels.discord.token="${DISCORD_BOT_TOKEN}"` and wire `fleet.bots.<bot>.profile.secretEnv.DISCORD_BOT_TOKEN = "<secretName>"`
