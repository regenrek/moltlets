...
All gateway config lives in `fleet/clawlets.json`.
...
- `hosts.<host>.gateways.<gatewayId>.openclaw` (raw OpenClaw config object)
...
<Callout type="warning" title="Avoid openclaw.* for typed surfaces">
  ...
  Use the first-class keys on the gateway instead.
</Callout>
...
fetch the schema from a running gateway, which requires `hosts.<host>.targetHost` and an
...
- Discord: set `hosts.<host>.gateways.<gatewayId>.channels.discord.token="${DISCORD_BOT_TOKEN}"` and wire `hosts.<host>.gateways.<gatewayId>.profile.secretEnv.DISCORD_BOT_TOKEN = "<secretName>"`
