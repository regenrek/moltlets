...
At deploy time, the Nix layer renders a per-bot Clawdbot config file so services
consume a single, validated JSON file:

- Nix generates `/run/secrets/rendered/clawdbot-<bot>.json`

## Canonical bot config (typed surfaces + Clawdbot schema)

Each bot has a single input object with typed surfaces plus a raw Clawdbot passthrough:
...
- `fleet.bots.<bot>.clawdbot` (JSON passthrough for advanced config)
...
- gateway auth token env: `/srv/clawdbot/<bot>/credentials/gateway.env`
...
Host ops snapshots (no secrets) are opt‑in (`services.clawdbotFleet.opsSnapshot.enable = true`).
