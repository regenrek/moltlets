...
This page explains the per‑gateway configuration surface (bots in the UI): workspaces,
skills, and how gateway profiles map into the Nix layer.
...
- `fleet.gateways.<id>.channels`
- `fleet.gateways.<id>.agents`
- `fleet.gateways.<id>.hooks`
- `fleet.gateways.<id>.skills`
- `fleet.gateways.<id>.plugins`
- `fleet.gateways.<id>.openclaw` (JSON passthrough for advanced config)
...
- per‑bot overrides: `fleet.gateways.<bot>.profile.secretEnv.<ENV_VAR> = "<secretName>"`
...
Gateway ports are auto-assigned from `fleet.gatewayOrder`, which keeps port allocation
deterministic across deploys.

Per‑bot override (example):

```bash
clawlets config set --path fleet.gateways.melinda.profile.gatewayPort --value 18819
```
...
```bash
clawlets config set --path fleet.gateways.melinda.agents.defaults.model.primary --value zai/glm-4.7
clawlets config set --path fleet.gateways.melinda.agents.defaults.model.fallbacks --value-json '["anthropic/claude-opus-4-5"]'
```
...
clawlets config set --path fleet.codex.gateways --value-json '["gunnar","maren"]'
```

Then allow bundled `coding-agent` for those bots:

```bash
clawlets config set --path fleet.gateways.gunnar.skills.allowBundled --value-json '["github","coding-agent"]'
clawlets config set --path fleet.gateways.maren.skills.allowBundled --value-json '["github","brave-search","coding-agent"]'
```
...
Requires at least one bot with GitHub App config (`fleet.gateways.<bot>.profile.github.*`).
