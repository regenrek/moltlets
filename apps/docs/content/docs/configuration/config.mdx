...
Clawdbot config should use `${ENV_VAR}` (uppercase/underscores). Clawlets scans the
effective bot config (typed fields + `fleet.bots.<bot>.clawdbot` passthrough) for
