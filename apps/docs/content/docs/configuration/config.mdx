...
- `baseFlake` (optional): flake URI for the evaluation source (e.g. `github:<owner>/<repo>`).
  Leave empty to use the project repo’s `origin` remote by default.
  Set this only when you intentionally want Nix to evaluate a different repo.
