...
The schema evolves. `schemaVersion` is currently `1`.

Clawlets intentionally does not accept multiple schemas at runtime. To upgrade an existing
repo, run:

- `clawlets config migrate --write`
- `clawlets config validate --host <host>`
## Example
