...
- `hosts.<host>`: host entries keyed by host name (includes the bots that run on that host)
...
also define which bots run on this host.

- `botsOrder`: ordered bot ids for this host (deterministic ports/services)
- `bots.<botId>`: per-bot config object (bots are **host-scoped**)
  - `profile`: clawlets/template infra knobs (systemd/env/secrets/limits)
    ...
    - other keys are forwarded into Nix `services.openclawFleet.gatewayProfiles.<botId>`
...
effective bot config (typed fields + `hosts.<host>.bots.<botId>.openclaw` passthrough) for
...
If you set `hosts.<host>.bots.<botId>.hooks.*Secret` or `hosts.<host>.bots.<botId>.skills.entries.*.apiKeySecret`,
...
Schema versions change as the configuration model evolves.

- v17: bots are host-scoped (manual update; no auto-migration):
  - remove `fleet.gatewayOrder` / `fleet.gateways`
  - add `hosts.<host>.botsOrder` / `hosts.<host>.bots`
  - bot ids are only unique within a host; address bots as `(host, botId)`
  - replace `fleet.codex.gateways` with `fleet.codex.bots` (if used)
- v15: config key renamed to `openclaw` (use `hosts.<host>.bots.<botId>.openclaw`, not `clawdbot`)
